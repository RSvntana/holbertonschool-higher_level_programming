...
class TestBase(unittest.TestCase):
    ...
    def test_load_from_file(self):
        ...
        b1 = Base()
        b2 = Base(42)
        list_base_input = [b1, b2]

        Base.save_to_file(list_base_input)
        list_base_output = Base.load_from_file()

        self.assertEqual(len(list_base_input), len(list_base_output))
        self.assertIsInstance(list_base_output[0], Base)
        self.assertIsInstance(list_base_output[1], Base)
        self.assertEqual(list_base_output[0].id, b1.id)
        self.assertEqual(list_base_output[1].id, b2.id)
